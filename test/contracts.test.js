...
describe("Contracts...", () => {
    it("Add client profile clause to where object", async() => {
        let where = addProfileClause({}, 'client', 2);
        expect(where.ClientId).equal(2);
    });

    it("Add contractor profile clause to where object", async() => {
        let where = addProfileClause({}, 'contractor', 2);
        expect(where.ContractorId).equal(2);
    });

    it("Get null as the where clause for unkown profile type", async () => {
        const where = addProfileClause({}, 'unkownProfileType', 2);
        expect(where).to.be.null;
    });

    it("Get where clause for contracts by contract id correctly", async () => {
        const where = Contracts.getWhereClause(1, 'client', 2);
        expect(where.id).equal(1);
    });

    it("Get where clause for all contracts not terminated as client correctly", async () => {
        const where = Contracts.getWhereAllNoTerm('client', 2);
        expect(where.status[Op.not]).equal('terminated');
    });
    
    it("Get where clause for all contracts that are active as client correctly", async () => {
